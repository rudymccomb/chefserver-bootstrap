directory node[:chefserver_bootstrap][:backup][:dir] do
  recursive true
  owner 'opscode-pgsql'
  mode  '0755'
end
...
node[:chefserver_bootstrap][:backup_gems].each_pair do |gem_name, gem_version|
...
directory node[:chefserver_bootstrap][:configuration_directory] do
  recursive true
  owner 'root'
  mode 0700
end

file File.join(node[:chefserver_bootstrap][:configuration_directory], 'backup.json') do
  content Chef::JSONCompat.to_json_pretty(
    node[:chefserver_bootstrap][:backup].merge(
...
cron 'Chef Server Backups' do
  ...
  node[:chefserver_bootstrap][:backup][:schedule].each do |k,v|
    send(k,v)
  end
